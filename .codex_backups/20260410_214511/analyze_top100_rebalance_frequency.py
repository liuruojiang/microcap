...
def get_price_limit_ratio(symbol: str, trade_date: pd.Timestamp) -> float:
...
def build_tradeability_series(
    ...
    ret_series = close_calendar.pct_change(fill_method=None).astype(float)
    ...
    if not ohlc_lookup.empty:
        prev_close = close_calendar.shift(1)
        for dt in trading_dates[tradeable.to_numpy()]:
            if pd.isna(ohlc_lookup.loc[dt].get("open")):
                continue
            up_locked, down_locked = detect_limit_locks(symbol, pd.Timestamp(dt), prev_close.loc[dt], ohlc_lookup.loc[dt])
            if up_locked:
...
def load_cache_panels(
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as pool:
        futures = {
            pool.submit(load_symbol_cache, symbol, trading_dates, cap_dates): symbol for symbol in symbols
        }
            ...
            turnover_rows.append(
                {
                    ...
                    "effective_date": pd.Timestamp(dt),
                    "exit_count": sells,
                    "entry_count": buys,
