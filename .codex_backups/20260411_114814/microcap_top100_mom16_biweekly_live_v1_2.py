...
def patched_live_module():
    ...
    try:
        ...
        live_mod.STRATEGY_TITLE = "Top100 Microcap Mom16 Biweekly v1.2"
        yield live_mod
    finally:
...
def build_performance_payload(ret: pd.Series) -> dict[str, object]:
    ...
    plt.title("Top100 Microcap Mom16 Biweekly v1.2")
    plt.ylabel("NAV")
    plt.grid(alpha=0.25)
    plt.tight_layout()
...
def generate_v1_2_outputs() -> tuple[dict[str, object], pd.DataFrame, pd.DataFrame]:
    ...
    summary["version_note"] = (
        "Same as v1.1 (0.8x hedge), plus practical NAV throttle 4/8 with "
        "0.85x/0.70x scale and recover 3%."
    )
