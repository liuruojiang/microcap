...
RUN_FOLDER = ROOT / "quant_param_scan_runs" / "20260523_microcap_v2_3_microcap_only_lookback_thr40"
LOOKBACKS = (5, 8, 10, 12, 15, 17, 20, 24, 30, 40, 50, 60, 80, 100, 120)
...
def _label(lookback: int) -> str:
    return f"lb{int(lookback)}_hl{HALFLIFE:g}_thr0p40".replace(".", "p")
...
def _score_frames(close_df: pd.DataFrame, official_index: pd.DatetimeIndex) -> tuple[pd.DataFrame, dict[int, pd.DataFrame]]:
    ...
    for lookback in LOOKBACKS:
        score = v23.log_wls_score_and_r2(micro_nav, lookback=int(lookback), halflife=HALFLIFE)
        valid = score["annualized_log_wls_score"].notna() & score["log_wls_r2"].notna()
        valid_index = pd.DatetimeIndex(score.index[valid])
        common_index = pd.DatetimeIndex(common_index.intersection(valid_index))
        frames[int(lookback)] = score
...
def _build_candidate_gross(base: pd.DataFrame, score_frame: pd.DataFrame, lookback: int) -> pd.DataFrame:
...
def _apply_cost(gross: pd.DataFrame, turnover_df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["strategy_variant"] = "v2_3_microcap_only_lookback_thr40_cost_only"
...
def _scan(run_folder: Path) -> tuple[pd.DataFrame, pd.DataFrame, dict[str, Any]]:
    ...
    base, score_frames = _score_frames(close_df, pd.DatetimeIndex(official_out.index))
    summary_rows: list[dict[str, Any]] = []
    wide_rows: list[dict[str, Any]] = []
    verification_rows: list[dict[str, Any]] = []

    for lookback in LOOKBACKS:
        label = _label(lookback)
        gross = _build_candidate_gross(base, score_frames[int(lookback)], int(lookback))
    ...
    baseline_label = _label(v23.LOOKBACK)
    best_label = str(wide.sort_values("decision_score", ascending=False).iloc[0]["candidate"])
    wide.loc[wide["candidate"].eq(baseline_label), "decision_hint"] = "current_lookback_baseline"
...
def _write_record(run_folder: Path, wide: pd.DataFrame, context: dict[str, Any], meta: dict[str, Any]) -> None:
    ...
    lines = [
        ...
        "- Parameter group: `lookback_window`",
        "- Scan type: signal_parameter_grid",
        f"- Repo or workspace path: `{ROOT}`",
        "- Target entrypoint: `scripts/run_microcap_v2_3_microcap_only_lookback_scan.py`",
        ...
        "## Research Question",
        "",
        f"- Baseline: microcap-only log-WLS momentum with lookback `{v23.LOOKBACK}`, halflife `{HALFLIFE:g}`, threshold `{MOMENTUM_THRESHOLD:.2f}`, no R2 gate.",
        f"- Candidate grid: lookbacks `{list(LOOKBACKS)}`.",
        "- Decision target: identify whether the WLS lookback window improves after the threshold/R2 first layer.",
        ...
        "- Date alignment: all candidates share the same valid date index across every scanned lookback.",
        ...
        "",
        "```powershell",
        f"python scripts/run_microcap_v2_3_microcap_only_lookback_scan.py --run-folder {run_folder}",
        f"python C:\\Users\\Administrator.DESKTOP-95I7VVU\\.codex\\skills\\quant-param-scan\\scripts\\finalize_quant_param_scan_run.py {run_folder} --decision research_only_watchlist_lookback --stability-label second_layer_lookback_scan",
        ...
        "",
        "- Label: second_layer_lookback_scan.",
        ...
        "## Decision",
        "",
        "- Decision: research_only_watchlist_lookback.",
        "- Recommended next action: compare local plateau around the best lookback before adding another layer.",
    ]
...
def run(run_folder: Path) -> None:
    ...
    with command_log.open("a", encoding="utf-8") as fh:
        fh.write(f"\n[{pd.Timestamp.now().isoformat()}] python scripts/run_microcap_v2_3_microcap_only_lookback_scan.py --run-folder {run_folder}\n")
    ...
    meta = {
        ...
        "decision": "research_only_watchlist_lookback",
        "stability_label": "second_layer_lookback_scan",
