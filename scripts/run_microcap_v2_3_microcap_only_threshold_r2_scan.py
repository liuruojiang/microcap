...
RUN_FOLDER = ROOT / "quant_param_scan_runs" / "20260523_microcap_v2_3_microcap_only_threshold_r2_gate"
MOMENTUM_THRESHOLDS = (0.00, 0.05, 0.10, 0.15, 0.20, 0.30, 0.40)
R2_GATES: tuple[float | None, ...] = (None, 0.05, 0.10, 0.20, 0.30, 0.40)
...
def _label(threshold: float, r2_gate: float | None) -> str:
    thr = f"{threshold:.2f}".replace(".", "p")
    r2 = "none" if r2_gate is None else f"{r2_gate:.2f}".replace(".", "p")
    return f"thr{thr}_r2{r2}"
...
def _build_candidate_gross(base: pd.DataFrame, threshold: float, r2_gate: float | None) -> pd.DataFrame:
    score = pd.to_numeric(base["annualized_log_wls_score"], errors="coerce")
    r2 = pd.to_numeric(base["log_wls_r2"], errors="coerce")
    r2_ok = pd.Series(True, index=base.index) if r2_gate is None else r2.ge(float(r2_gate))
    signal_on = score.gt(float(threshold)) & r2_ok
    ...
    out = pd.DataFrame(
        {
            ...
            "r2_gate": np.nan if r2_gate is None else float(r2_gate),
            "futures_drag": 0.0,
...
def _apply_cost(gross: pd.DataFrame, turnover_df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["strategy_variant"] = "v2_3_microcap_only_threshold_r2_cost_only"
...
def _scan(run_folder: Path) -> tuple[pd.DataFrame, pd.DataFrame, dict[str, Any]]:
    ...
    for threshold in MOMENTUM_THRESHOLDS:
        for r2_gate in R2_GATES:
            label = _label(threshold, r2_gate)
            gross = _build_candidate_gross(base, threshold, r2_gate)
    ...
    baseline_label = _label(0.0, None)
    best_label = str(wide.sort_values("decision_score", ascending=False).iloc[0]["candidate"])
    wide.loc[wide["candidate"].eq(baseline_label), "decision_hint"] = "microcap_only_baseline"
...
def _write_record(run_folder: Path, wide: pd.DataFrame, context: dict[str, Any], meta: dict[str, Any]) -> None:
    ...
    lines = [
        ...
        "- Parameter group: `momentum_threshold_r2_gate`",
        "- Scan type: signal_parameter_grid",
        f"- Repo or workspace path: `{ROOT}`",
        "- Target entrypoint: `scripts/run_microcap_v2_3_microcap_only_threshold_r2_scan.py`",
        ...
        "## Research Question",
        "",
        "- Baseline: microcap-only original log-WLS momentum, threshold 0, no R2 gate.",
        f"- Candidate grid: momentum thresholds `{list(MOMENTUM_THRESHOLDS)}` crossed with R2 gates `{list(R2_GATES)}`.",
        "- Decision target: identify whether a stricter momentum/R2 condition improves recent risk-adjusted behavior before adding other layers.",
        ...
        "- Signal object: log-WLS score and R2 on microcap Top100 NAV only.",
        ...
        "- Removed: ZZ1000 hedge, futures drag, target-vol scaling, financing, cash-day yield, spread NAV, exit buffer, peak decay, R2 from official spread model, and broad-volume overlays.",
        ...
        "",
        "```powershell",
        f"python scripts/run_microcap_v2_3_microcap_only_threshold_r2_scan.py --run-folder {run_folder}",
        f"python C:\\Users\\Administrator.DESKTOP-95I7VVU\\.codex\\skills\\quant-param-scan\\scripts\\finalize_quant_param_scan_run.py {run_folder} --decision research_only_watchlist_threshold_r2 --stability-label first_layer_scan",
        ...
        "## Decision",
        "",
        "- Decision: research_only_watchlist_threshold_r2.",
        "- Recommended next action: inspect whether the best row is a broad plateau before adding the next condition layer.",
    ]
...
def run(run_folder: Path) -> None:
    ...
    with command_log.open("a", encoding="utf-8") as fh:
        fh.write(f"\n[{pd.Timestamp.now().isoformat()}] python scripts/run_microcap_v2_3_microcap_only_threshold_r2_scan.py --run-folder {run_folder}\n")
    ...
    meta = {
        ...
        "repo_root": str(ROOT),
        "entrypoint": "scripts/run_microcap_v2_3_microcap_only_threshold_r2_scan.py",
        "git_branch": _git(["branch", "--show-current"]),
        "git_commit": _git(["rev-parse", "HEAD"]),
        "git_status_before": _git(["status", "--short"]),
        "git_status_after": _git(["status", "--short"]),
        "scan_type": "signal_parameter_grid",
        "parameter_group": "momentum_threshold_r2_gate",
        "baseline": {
        ...
        "decision": "research_only_watchlist_threshold_r2",
        "stability_label": "first_layer_scan",
