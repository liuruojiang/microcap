...
RUN_FOLDER = ROOT / "quant_param_scan_runs" / "20260523_microcap_v2_3_microcap_only_entry_exit_threshold"
ENTRY_THRESHOLDS = (0.00, 0.10, 0.20, 0.30, 0.40, 0.50, 0.60)
EXIT_THRESHOLDS = (-0.20, -0.10, 0.00, 0.10, 0.20, 0.30, 0.40, 0.50, 0.60)
...
def _label(entry_threshold: float, exit_threshold: float) -> str:
    return f"entry{_fmt_threshold(entry_threshold)}_exit{_fmt_threshold(exit_threshold)}"
...
def _build_candidate_gross(base: pd.DataFrame, entry_threshold: float, exit_threshold: float) -> pd.DataFrame:
    ...
    for dt in base.index:
        ...
        if pd.isna(current_score):
            ...
            next_active = float(current_score) > float(exit_threshold)
        else:
            next_active = float(current_score) > float(entry_threshold)
...
def _apply_cost(gross: pd.DataFrame, turnover_df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["strategy_variant"] = "v2_3_microcap_only_entry_exit_threshold_cost_only"
...
def _scan(run_folder: Path) -> tuple[pd.DataFrame, pd.DataFrame, dict[str, Any]]:
    ...
    for entry_threshold, exit_threshold in _candidate_pairs():
        label = _label(entry_threshold, exit_threshold)
        gross = _build_candidate_gross(base, entry_threshold, exit_threshold)
        ...
        wide: dict[str, Any] = {
            ...
            "entry_threshold": float(entry_threshold),
            "exit_threshold": float(exit_threshold),
            "hysteresis_width": float(entry_threshold - exit_threshold),
    ...
    original_label = _label(0.0, 0.0)
    unified_40_label = _label(0.4, 0.4)
    best_label = str(wide.sort_values("decision_score", ascending=False).iloc[0]["candidate"])
    wide.loc[wide["candidate"].eq(original_label), "decision_hint"] = "original_zero_threshold"
    wide.loc[wide["candidate"].eq(unified_40_label), "decision_hint"] = "prior_best_unified_threshold"
    ...
    return summary, wide, {
        ...
        "unified_40_label": unified_40_label,
...
def _write_record(run_folder: Path, wide: pd.DataFrame, context: dict[str, Any], meta: dict[str, Any]) -> None:
    ...
    best = ordered.iloc[0]
    original = wide.loc[wide["candidate"].eq(context["original_label"])].iloc[0]
    unified_40 = wide.loc[wide["candidate"].eq(context["unified_40_label"])].iloc[0]
    ...
    lines = [
        ...
        "- Parameter group: `entry_exit_threshold`",
        "- Scan type: signal_parameter_grid",
        f"- Repo or workspace path: `{ROOT}`",
        "- Target entrypoint: `scripts/run_microcap_v2_3_microcap_only_entry_exit_threshold_scan.py`",
        ...
        "## Research Question",
        "",
        "- Baseline: microcap-only original log-WLS momentum, entry 0, exit 0.",
        f"- Candidate grid: entry thresholds `{list(ENTRY_THRESHOLDS)}` crossed with exit thresholds `{list(EXIT_THRESHOLDS)}`, constrained to `exit <= entry`.",
        "- Decision target: test whether hysteresis improves the first-layer microcap-only signal before adding any other conditions.",
        ...
        "- Signal object: annualized log-WLS score on microcap Top100 NAV only.",
        "- State rule: cash enters only when score > entry threshold; existing holdings continue only while score > exit threshold.",
        ...
        "- Removed: ZZ1000 hedge, futures drag, target-vol scaling, financing, cash-day yield, spread NAV, R2 gate, peak decay, and broad-volume overlays.",
        ...
        "",
        "```powershell",
        f"python scripts/run_microcap_v2_3_microcap_only_entry_exit_threshold_scan.py --run-folder {run_folder}",
        f"python C:\\Users\\Administrator.DESKTOP-95I7VVU\\.codex\\skills\\quant-param-scan\\scripts\\finalize_quant_param_scan_run.py {run_folder} --decision research_only_watchlist_entry_exit_threshold --stability-label first_layer_hysteresis_scan",
        ...
        "## Full-Sample Results",
        "",
        f"- Original `{context['original_label']}`: annual return {original['ann_return_full']:.4%}, max drawdown {original['max_dd_full']:.4%}, Sharpe {original['sharpe_repo_full']:.3f}.",
        f"- Unified-40 `{context['unified_40_label']}`: annual return {unified_40['ann_return_full']:.4%}, max drawdown {unified_40['max_dd_full']:.4%}, Sharpe {unified_40['sharpe_repo_full']:.3f}.",
        ...
        "",
        "- Label: first_layer_hysteresis_scan.",
        ...
        "## Decision",
        "",
        "- Decision: research_only_watchlist_entry_exit_threshold.",
        "- Recommended next action: inspect local plateau around the best candidate before adding the next condition layer.",
    ]
...
def run(run_folder: Path) -> None:
    ...
    with command_log.open("a", encoding="utf-8") as fh:
        fh.write(f"\n[{pd.Timestamp.now().isoformat()}] python scripts/run_microcap_v2_3_microcap_only_entry_exit_threshold_scan.py --run-folder {run_folder}\n")
    ...
    meta = {
        ...
        "repo_root": str(ROOT),
        "entrypoint": "scripts/run_microcap_v2_3_microcap_only_entry_exit_threshold_scan.py",
        "git_branch": _git(["branch", "--show-current"]),
        "git_commit": _git(["rev-parse", "HEAD"]),
        "git_status_before": _git(["status", "--short"]),
        "git_status_after": _git(["status", "--short"]),
        "scan_type": "signal_parameter_grid",
        "parameter_group": "entry_exit_threshold",
        "baseline": {
            "candidate": context["original_label"],
            "entry_threshold": 0.0,
            "exit_threshold": 0.0,
            "lookback": int(v23.LOOKBACK),
            "halflife": float(v23.HALFLIFE),
        ...
        "decision": "research_only_watchlist_entry_exit_threshold",
        "stability_label": "first_layer_hysteresis_scan",
