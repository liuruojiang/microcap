...
RUN_FOLDER = ROOT / "quant_param_scan_runs" / "20260523_microcap_top100_v2_3_derived_microcap_only_momentum_decay_momentum_decay_exit_reentry"
...
DECAY_THRESHOLDS = (0.10, 0.20, 0.30, 0.40, 0.50, 0.60)
RECOVERY_OF_PEAK_THRESHOLDS = (0.50, 0.60, 0.70, 0.80, 0.90, 1.00)
...
def _label(decay_threshold: float | None, recovery_of_peak: float | None) -> str:
    if decay_threshold is None:
        return "no_momentum_decay"
    return f"decay{int(round(decay_threshold * 100)):02d}_rec{int(round(float(recovery_of_peak) * 100)):03d}"
...
def _apply_momentum_decay_overlay(
    shadow: pd.DataFrame,
    decay_threshold: float | None,
    recovery_of_peak: float | None,
) -> pd.DataFrame:
    ...
    if decay_threshold is None:
        out["momentum_decay_threshold"] = np.nan
        out["momentum_recovery_of_peak_threshold"] = np.nan
        out["momentum_peak_since_entry"] = np.nan
        out["momentum_peak_ratio"] = np.nan
        out["momentum_decay_risk_off"] = False
        out["momentum_decay_exit_triggered"] = False
        out["momentum_decay_reentry_triggered"] = False
        out["actual_execution_scale"] = shadow_scale
        out["overlay_trade_cost"] = 0.0
        return out

    decay_thr = float(decay_threshold)
    rec_ratio = float(recovery_of_peak)
    keep_ratio = 1.0 - decay_thr
    if decay_thr <= 0.0 or decay_thr >= 1.0:
        raise ValueError("decay_threshold must be in (0, 1)")
    if rec_ratio < keep_ratio:
        raise ValueError("recovery_of_peak must be >= 1 - decay_threshold")
    ...
    for dt in out.index:
        ...
        if not next_active or pd.isna(current_score):
            ...
            peak = score_value if peak is None else max(float(peak), score_value)
            peak_value = float(peak)
            peak_ratio = score_value / peak_value if peak_value > 0 else np.nan
            if risk_off:
                if peak_value > 0 and score_value >= peak_value * rec_ratio:
                    risk_off = False
                    reentry_trigger = True
            else:
                if peak_value > 0 and score_value <= peak_value * keep_ratio:
                    risk_off = True
                    exit_trigger = True
    ...
    out["momentum_decay_threshold"] = decay_thr
    out["momentum_recovery_of_peak_threshold"] = rec_ratio
    out["momentum_peak_since_entry"] = pd.Series(peak_values, index=out.index, dtype=float)
    out["momentum_peak_ratio"] = pd.Series(peak_ratios, index=out.index, dtype=float)
    out["momentum_decay_risk_off"] = pd.Series(risk_off_flags, index=out.index, dtype=bool)
    out["momentum_decay_exit_triggered"] = pd.Series(exit_flags, index=out.index, dtype=bool)
    out["momentum_decay_reentry_triggered"] = pd.Series(reentry_flags, index=out.index, dtype=bool)
...
def _candidate_grid() -> list[tuple[float | None, float | None]]:
    ...
    for decay_thr in DECAY_THRESHOLDS:
        keep_ratio = 1.0 - float(decay_thr)
        for rec_ratio in RECOVERY_OF_PEAK_THRESHOLDS:
            if float(rec_ratio) >= keep_ratio:
                pairs.append((float(decay_thr), float(rec_ratio)))
    return pairs
...
def _scan(run_folder: Path) -> tuple[pd.DataFrame, pd.DataFrame, dict[str, Any]]:
    ...
    for decay_thr, rec_ratio in _candidate_grid():
        label = _label(decay_thr, rec_ratio)
        out = _apply_momentum_decay_overlay(shadow, decay_thr, rec_ratio)
        ...
        verification_rows.append(
            {
                ...
                "exit_count": int(out["momentum_decay_exit_triggered"].fillna(False).sum()),
                "reentry_count": int(out["momentum_decay_reentry_triggered"].fillna(False).sum()),
                "risk_off_days": int(out["momentum_decay_risk_off"].fillna(False).sum()),
                "final_nav": float(out["nav_net"].iloc[-1]),
            }
        )
        wide: dict[str, Any] = {
            "candidate": label,
            "momentum_decay_threshold": np.nan if decay_thr is None else float(decay_thr),
            "momentum_recovery_of_peak_threshold": np.nan if rec_ratio is None else float(rec_ratio),
            ...
            "momentum_decay_exit_count_full": int(out["momentum_decay_exit_triggered"].fillna(False).sum()),
            "momentum_decay_reentry_count_full": int(out["momentum_decay_reentry_triggered"].fillna(False).sum()),
            "momentum_decay_risk_off_days_full": int(out["momentum_decay_risk_off"].fillna(False).sum()),
        ...
        for segment, (start, end) in _window_slices(pd.DatetimeIndex(out.index)).items():
            ...
            row = {
                ...
                "momentum_decay_exit_count": int(part["momentum_decay_exit_triggered"].fillna(False).sum()),
                "momentum_decay_reentry_count": int(part["momentum_decay_reentry_triggered"].fillna(False).sum()),
                "momentum_decay_risk_off_days": int(part["momentum_decay_risk_off"].fillna(False).sum()),
            ...
            for metric in (
                ...
                "avg_execution_scale",
                "max_execution_scale",
                "momentum_decay_exit_count",
                "momentum_decay_reentry_count",
                "momentum_decay_risk_off_days",
    ...
    wide.loc[wide["candidate"].eq("no_momentum_decay"), "decision_hint"] = "tv30_max1p3_no_momentum_decay"
    ...
    verification.to_csv(run_folder / "momentum_decay_sanity_checks.csv", index=False, encoding="utf-8")
    return summary, wide, {
        "reference_summary": reference_summary,
        "metrics_start": str(pd.Timestamp(base.index.min()).date()),
        "metrics_end": str(pd.Timestamp(base.index.max()).date()),
        "rows": int(len(base)),
        "baseline_label": "no_momentum_decay",
...
def _write_record(run_folder: Path, wide: pd.DataFrame, context: dict[str, Any], meta: dict[str, Any]) -> None:
    ...
    top = ordered.head(15)[
        [
            ...
            "ann_return_last_1y",
            "max_dd_last_1y",
            "momentum_decay_exit_count_full",
            "momentum_decay_reentry_count_full",
            "momentum_decay_risk_off_days_full",
    ...
    lines = [
        ...
        "- Sleeve or subsystem: microcap-only momentum-decay",
        "- Parameter group: `momentum_decay_exit_reentry`",
        "- Scan type: momentum_peak_decay_overlay_grid",
        f"- Repo or workspace path: `{ROOT}`",
        "- Target entrypoint: `scripts/run_microcap_v2_3_microcap_only_momentum_decay_scan.py`",
        ...
        "- Candidate grid: exit when momentum score decays from the current holding episode peak; recover when the score returns to a chosen percentage of that peak.",
        f"- Decay thresholds: `{list(DECAY_THRESHOLDS)}`.",
        f"- Recovery-of-peak thresholds: `{list(RECOVERY_OF_PEAK_THRESHOLDS)}` where recovery >= remaining peak ratio.",
        "- Source-change rule: `research_only_no_source_change`.",
        "- Required windows: full, last_10y, last_5y, last_3y, last_1y.",
        "",
        "## Implementation Anchor",
        "",
        "- Shadow path: baseline TV30/max1.3 strategy without momentum-decay overlay.",
        "- Momentum peak: maximum `annualized_log_wls_score` observed during the current base holding episode.",
        "- Exit trigger: after close on T, if score <= peak * (1 - decay_threshold), set overlay risk-off for T+1.",
        "- Recovery trigger: after close on T, if score >= peak * recovery_of_peak and the base signal is active, resume following the shadow strategy from T+1.",
        ...
        "- Added: overlay microcap one-side trading cost on momentum-decay risk-off/risk-on scale changes.",
        "- No hedge leg, futures drag, or hedge turnover.",
        "",
        "## Commands",
        "",
        "```powershell",
        f"python scripts/run_microcap_v2_3_microcap_only_momentum_decay_scan.py --run-folder {run_folder}",
        f"python C:\\Users\\Administrator.DESKTOP-95I7VVU\\.codex\\skills\\quant-param-scan\\scripts\\finalize_quant_param_scan_run.py {run_folder} --decision research_only_watchlist_momentum_decay --stability-label momentum_decay_first_pass",
        ...
        "",
        "- Label: momentum_decay_first_pass.",
        ...
        "## Decision",
        "",
        "- Decision: research_only_watchlist_momentum_decay.",
        "- Recommended next action: inspect whether any decay/recovery rows dominate the no-overlay baseline across all required windows.",
    ]
...
def run(run_folder: Path) -> None:
    ...
    with command_log.open("a", encoding="utf-8") as fh:
        fh.write(f"\n[{pd.Timestamp.now().isoformat()}] python scripts/run_microcap_v2_3_microcap_only_momentum_decay_scan.py --run-folder {run_folder}\n")
    ...
    meta = {
        ...
        "subsystem": "microcap-only momentum-decay",
        "repo_root": str(ROOT),
        "entrypoint": "scripts/run_microcap_v2_3_microcap_only_momentum_decay_scan.py",
        "git_branch": _git(["branch", "--show-current"]),
        "git_commit": _git(["rev-parse", "HEAD"]),
        "git_status_before": _git(["status", "--short"]),
        "git_status_after": _git(["status", "--short"]),
        "scan_type": "momentum_peak_decay_overlay_grid",
        "parameter_group": "momentum_decay_exit_reentry",
        "baseline": {
        ...
        "cost_model": {
            ...
            "overlay_trade_cost_model": "one_side_microcap_turnover_only_when_momentum_decay_overlay_changes_risk_state",
        ...
        "outputs": {
            ...
            "momentum_decay_sanity_checks": str(run_folder / "momentum_decay_sanity_checks.csv"),
        },
        "decision": "research_only_watchlist_momentum_decay",
        "stability_label": "momentum_decay_first_pass",
