...
def v2_5_rewrite_audit_matches_approved_lineage_migration(
    ...
    try:
        ...
        prev = v2_0.base_mod._normalise_dated_frame(previous, "approved v2.5 migration previous")
        cand = v2_0.base_mod._normalise_dated_frame(candidate, "approved v2.5 migration candidate")
        expected = {
            "schema_version": 1,
            "version": VERSION,
            "approved": True,
...
def _write_v2_5_lineage_migration_diagnostics(report_path: Path, audit_path: Path) -> Path:
    ...
    diagnostics = {
        ...
        "note": "One-time exact-hash v2.5 migration after the audited v2.0 ST and historical-universe repair.",
    }
    diagnostics_path = OUTPUT_DIR / f"{OUTPUT_PREFIX}_historical_rewrite_diagnostics.json"
    _atomic_write_text(diagnostics_path, _json_dumps(diagnostics), encoding="utf-8")
    return diagnostics_path
    ...
    if previous_path.exists():
        ...
        try:
            ...
            if strategy_promotion_matches(
                ...
                diagnostics_path = _write_v2_5_lineage_migration_diagnostics(Path(report_path), audit_path)
                rewrite_audit_status = {
