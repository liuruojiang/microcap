...
import analyze_microcap_rsrs_proxy_zz1000_hedge as single_rsrs_mod
...
OUT_PREFIX = "microcap_relative_rsrs_n20_m80_zz1000_hedged"
...
def load_hedge_series() -> pd.Series:
    panel_path = single_rsrs_mod.resolve_panel_path()
...
def build_summary(result: pd.DataFrame, close_df: pd.DataFrame) -> dict[str, object]:
    ...
    return {
        ...
        "panel_path": str(single_rsrs_mod.resolve_panel_path()),
        "hedge_column": HEDGE_COLUMN,
        "reg_window": REG_WINDOW,
        "z_window": Z_WINDOW,
        "signal_col": SIGNAL_COL,
        "signal_price_definition": "relative_ohlc = microcap_ohlc / zz1000_close",
        "signal_note": (
            "Because the local CSI1000 panel only has daily close, relative OHLC is approximated "
            "with microcap open/high/low/close divided by same-day CSI1000 close."
        ),
...
def plot_nav(result: pd.DataFrame) -> None:
    ...
    plt.plot(result.index, result["nav"], linewidth=1.8, label="Relative RSRS N20 M80 / CSI1000 Hedge")
    plt.title("Microcap Relative RSRS + CSI1000 Hedge NAV")
    plt.grid(alpha=0.25)
    plt.legend()
    plt.tight_layout()
