...
OUT_GRID = OUTPUT_DIR / "microcap_rsrs_proxy_nm_no_threshold_scan.csv"
OUT_SUMMARY = OUTPUT_DIR / "microcap_rsrs_proxy_nm_no_threshold_summary.json"
OUT_HEATMAP = OUTPUT_DIR / "microcap_rsrs_proxy_nm_no_threshold_heatmap.png"

SIGNAL_COL = "right_skew"
N_WINDOWS = [6, 8, 10, 12, 14, 16, 18, 20, 24, 30, 40]
M_WINDOWS = [40, 60, 80, 100, 120, 160, 200, 250, 300, 400, 500, 600, 800, 1000, 1200]
            ...
            rows.append(
                {
                    ...
                    "annual_return": float(base.annual_return(result["strategy_nav"])),
                    "sharpe": float(base.sharpe_ratio(result["strategy_ret"])),
...
def build_summary(df: pd.DataFrame) -> dict:
    ...
    smallest_drawdown = df.sort_values(["max_drawdown", "sharpe"], ascending=[False, False]).iloc[0].to_dict()
    practical = df[
        (df["trades"] >= 10)
        & (df["position_days_pct"] >= 0.15)
        & (df["position_days_pct"] <= 0.85)
...
def plot_heatmap(df: pd.DataFrame) -> None:
    ...
    ax.set_title("Wind Microcap Proxy RSRS N-M Scan (Sharpe, no threshold)")
