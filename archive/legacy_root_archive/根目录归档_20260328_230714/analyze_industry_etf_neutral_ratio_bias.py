...
OUT_CLOSE_CSV = ROOT / "industry_etf_neutral_ratio_bias_close.csv"
OUT_NAV_CSV = ROOT / "industry_etf_neutral_ratio_bias_nav.csv"
OUT_SIGNAL_CSV = ROOT / "industry_etf_neutral_ratio_bias_latest_signal.csv"
OUT_SUMMARY_JSON = ROOT / "industry_etf_neutral_ratio_bias_summary.json"
OUT_PNG = ROOT / "industry_etf_neutral_ratio_bias_curve.png"
OUT_MD = ROOT / "mnt_industry_etf_neutral_ratio_bias_20260328.md"
...
def load_main_module():
    ...
    spec = importlib.util.spec_from_file_location("mntbot_v61_ratio_bias", MAIN_SCRIPT)
...
def run_rotation_market_neutral(mod, close_df: pd.DataFrame, name_map: dict[str, str]) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    for secid in etf_secids:
        score_df[secid] = mod.calc_bias_momentum(ratio_df[secid])
...
def plot_nav(nav: pd.Series) -> None:
    ...
    plt.plot(nav.index, nav.values, linewidth=1.8, label="Industry ETF Neutral Ratio Bias")
    plt.title("Industry ETF Neutral Ratio-Bias Strategy NAV")
    plt.legend()
    plt.grid(alpha=0.25)
    plt.tight_layout()
...
def build_summary(result: pd.DataFrame, latest_signal: pd.DataFrame, close_df: pd.DataFrame, name_map: dict[str, str]) -> dict:
    ...
    return {
        "strategy": "industry_etf_neutral_ratio_bias",
        "signal_mode": "bias on ETF / 中证500 ratio",
...
def main() -> None:
    ...
    result, score_df = run_rotation_market_neutral(mod, close_df, name_map)
