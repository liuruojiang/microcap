...
PROD_VS_VOL_WINDOW = 15          # 已实现波动率回看窗口(交易日)  # v6.1: 20→15 (鲁棒性优化)
PROD_VS_MAX_LEV = 1.5            # 最大杠杆倍数
PROD_VS_MIN_LEV = 0.5            # 最小仓位比例
PROD_VS_THRESHOLD = 0.10         # scale变动阈值 (Δscale ≥ 10% 才调整)  # v6.1: 5%→10% (鲁棒性优化)
...
def fetch_cn_kline(secid):
    ...
    if code.startswith('H'):
        # H-prefix全收益指数: csindex是唯一可靠源(已内置重试),EastMoney作备用
        sources = [
            ("csindex", lambda: _fetch_cn_csindex(code)),
...
def extract_subc_vs_rebalances(us_prod_daily, prod_sig_a, prod_sig_b):
...
class CombinedStrategyV58(CombinedStrategyBase):
    ...
    def _write_sub_c(self, msg, d, us_prod_daily):
        ...
        if not PROD_USE_TIMING:
            ...
            if _sub_c_pos:
                _actual_mkt_val_c = sum(
                    _pos_entry_value(_sub_c_pos.get(name, 0), _c_prices.get(name, 0))
                    for name in set(list(_sub_c_pos.keys()) + list(PROD_PORTFOLIO.keys()))
                )
                _target_val_c = _actual_mkt_val_c if _actual_mkt_val_c > 0 else _sub_c_capital
                if _target_val_c and _target_val_c > 0:
                    msg.write(f"\n📊 **仓位调整** (基于当前持仓市值${_target_val_c:,.0f}):\n")
                    msg.write("| ETF | 当前持仓 | 目标数量 | 调整 |\n|:-|--------:|--------:|-----:|\n")
                    _adj_etfs_c = set(list(_sub_c_pos.keys()) + list(PROD_PORTFOLIO.keys()))
                    for etf_c in sorted(_adj_etfs_c):
                        _raw_pos_c = _sub_c_pos.get(etf_c, 0)
                        price_c = _c_prices.get(etf_c, 0)
                        cur_shares_c = _pos_entry_shares(_raw_pos_c, price_c)
                        _w_base = PROD_PORTFOLIO[etf_c]["w"] if etf_c in PROD_PORTFOLIO else 0
                        _w_scaled = _w_base * _vs_scale
                        if price_c and price_c > 0 and _w_scaled > 0.001:
                            target_shares_c = int(_target_val_c * _w_scaled / price_c)
                        else:
                            target_shares_c = 0
                        adj_c = target_shares_c - cur_shares_c
                        if cur_shares_c == 0 and target_shares_c == 0:
                            continue
                        if adj_c > 0:
                            adj_str_c = f"+{adj_c:,} 买入"
                        elif adj_c < 0:
                            adj_str_c = f"{adj_c:,} 卖出"
                        else:
                            adj_str_c = "—"
                        ...
                        msg.write(f"| {etf_c} | {cur_display_c} | {target_shares_c:,} | {adj_str_c} |\n")
    ...
    def _handle_signal(self):
        ...
        vs_rebs = extract_subc_vs_rebalances(us_prod_daily, d.get("prod_sig_a"), d.get("prod_sig_b"))
    ...
    def _handle_performance(self, query, _forced_range=None):
        ...
        vs_rebs = extract_subc_vs_rebalances(us_prod_daily, prod_sig_a, prod_sig_b)
