...
OUT_CLOSE_CSV = ROOT / "industry_etf_neutral_ratio_abs60_weekly_r2_close.csv"
OUT_NAV_CSV = ROOT / "industry_etf_neutral_ratio_abs60_weekly_r2_nav.csv"
OUT_SIGNAL_CSV = ROOT / "industry_etf_neutral_ratio_abs60_weekly_r2_latest_signal.csv"
OUT_SUMMARY_JSON = ROOT / "industry_etf_neutral_ratio_abs60_weekly_r2_summary.json"
OUT_PNG = ROOT / "industry_etf_neutral_ratio_abs60_weekly_r2_curve.png"
OUT_MD = ROOT / "mnt_industry_etf_neutral_ratio_abs60_weekly_r2_20260328.md"
...
def load_main_module():
    ...
    spec = importlib.util.spec_from_file_location("mntbot_v61_ratio_abs60_weekly_r2", MAIN_SCRIPT)
...
def build_signal_frames(mod, close_df: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    for secid in etf_secids:
        ...
        score_df[secid] = calc_abs_momentum(ratio, LOOKBACK)
    ...
    for i in range(1, len(close_df)):
        ...
        if is_rebalance_day:
            ...
            if len(today_scores) > 0:
                ...
                best_r2 = r2_df.iloc[i].get(best, np.nan)
...
def plot_nav(nav: pd.Series) -> None:
    ...
    plt.plot(nav.index, nav.values, linewidth=1.8, label="Industry ETF Neutral Ratio Abs60 Weekly + R2")
    plt.title("Industry ETF Neutral Ratio Abs60 Weekly + R2 Filter NAV")
    plt.legend()
    plt.grid(alpha=0.25)
    plt.tight_layout()
...
def build_summary(
    ...
    return {
        "strategy": "industry_etf_neutral_ratio_abs60_weekly_r2",
        "signal_mode": "60d absolute momentum on ETF / 中证500 ratio, weekly rebalance, top1 + R² filter",
...
def main() -> None:
    ...
    result[["return", "nav", "holding", "next_target", "is_signal", "rebalance_day", "score", "r2"]].to_csv(
