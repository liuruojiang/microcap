...
# v5.8: 乖离动量 + R²过滤 + 国债指数 (替代v5.4双动量+MA拐头+冷却期)
...
# A股 全收益指数映射 (v5.8: 全部改用全收益指数，消除ETF前复权不一致问题)
...
# v5.8 多配对索引: 5指数 → C(5,2)=10配对, 全部从cn_dk_close读取(价格指数)
CN_DK_INDICES = {
...
def fetch_cn_kline(secid):
    code = secid.split('.')[1]
    sources = [("EastMoney", lambda: _fetch_cn_eastmoney(secid))]
    if code.startswith('H'):
        # H-prefix全收益指数: Sina不支持, 用csindex作为备用源
        sources.append(("csindex", lambda: _fetch_cn_csindex(code)))
    else:
        # ETF/普通指数: Sina作为备用源
        sources.append(("Sina", lambda: _fetch_cn_sina(secid)))
    last_err = None
...
def run_cn_strategy(close_df, equity_codes):
    """Sub-A V5.8: 乖离动量 + R²过滤 + 国债轮动 + 波动率缩放.
    v5.8变更: 乖离动量替代双动量排名, R²过滤替代MA拐头和冷却期, 国债加入轮动池, 波动率缩放控制风险.
    """
...
四个子策略可设置仓位:
Sub-A: A股轮动 - 全收益指数代码格式如 1.H20955(中证红利), 0.399606(创业板)
Sub-A-DK: A股多空 - 用指数期货/ETF实现, 标的: 中证1000(H00852), 上证50(H00016), 可填合约张数或ETF股数
...
class CombinedStrategyV58(CombinedStrategyBase):
        ...
        with _sm() as msg:
            ...
            if _sorted_live:
                ...
                if not np.isnan(_best_bm) and _best_bm <= 0:
                    ...
                    w(f"**R²过滤:** R²({CN_R2_WINDOW})={_r2v_best:.3f} -> {'**通过** ✅' if _r2_pass else '**未通过** ❌ -> 选次优或国债'}\n")
            # 成交量情绪（仅展示）
        ...
        with _sm() as msg:
            ...
            if _sorted_live2:
                ...
                if not np.isnan(_best2_bm) and _best2_bm <= 0:
                    ...
                    w(f"**R²过滤:** R²({CN_R2_WINDOW})={_r2v_best2:.3f} -> {'**通过** ✅' if _r2_pass2 else '**未通过** ❌ -> 选次优或国债'}\n")
                # 成交量情绪（仅展示）
    ...
    def _handle_params(self):
        with _sm() as msg:
            ...
            w(f"| R²门槛 | **{CN_R2_THRESHOLD}** | 权益指数需R²≥{CN_R2_THRESHOLD}才持有 |\n")
            w(f"| 国债 | **{CN_BOND_NAME}({CN_BOND_CODE})** | 避险资产，免R²过滤 |\n")
            ...
            w("3. 选乖离动量最高的资产\n")
            w(f"4. 权益指数需R²({CN_R2_WINDOW})≥{CN_R2_THRESHOLD}才持有，国债免R²过滤\n")
    ...
    def _handle_signal_history(self, query):
        ...
        cn_result, cn_dk_result, us_rot_result, *_ = \
            self._run_strategies(cn_close, cn_dk_close, us_rot_close, us_prod_daily)
        # v5.8: No MA filter, placeholder for history display
