...
OUT_CLOSE_CSV = ROOT / "industry_etf_neutral_bias_close.csv"
OUT_NAV_CSV = ROOT / "industry_etf_neutral_bias_nav.csv"
OUT_SIGNAL_CSV = ROOT / "industry_etf_neutral_bias_latest_signal.csv"
OUT_SUMMARY_JSON = ROOT / "industry_etf_neutral_bias_summary.json"
OUT_PNG = ROOT / "industry_etf_neutral_bias_curve.png"
OUT_MD = ROOT / "mnt_industry_etf_neutral_bias_20260328.md"

HEDGE_SECID = "1.000905"
HEDGE_NAME = "中证500价格指数"
CN_TRADING_DAYS = 244
SCORE_MODE = "bias"
TRADE_COST_PER_LEG = 0.0  # first-pass gross backtest; no ETF/futures cost or basis adjustment
MIN_ACTIVE_ETFS = 5
...
def calc_abs_momentum(series: pd.Series, lookback: int = 120) -> pd.Series:
    return series.div(series.shift(lookback)).sub(1.0)


def score_series(mod, series: pd.Series, mode: str) -> pd.Series:
    if mode == "bias":
        return mod.calc_bias_momentum(series)
    if mode == "abs":
        return calc_abs_momentum(series)
...
def run_rotation_market_neutral(mod, close_df: pd.DataFrame, name_map: dict[str, str], score_mode: str) -> tuple[pd.DataFrame, pd.DataFrame]:
    etf_secids = [c for c in close_df.columns if c != HEDGE_SECID]
    score_df = pd.DataFrame(index=close_df.index)
    for secid in etf_secids:
        score_df[secid] = score_series(mod, close_df[secid], score_mode)
    ...
    for i in range(1, len(close_df)):
        ...
        if is_signal and TRADE_COST_PER_LEG > 0:
            if holding == "cash" and target != "cash":
                ...
                cost = 2 * TRADE_COST_PER_LEG
        day_ret = (1.0 + asset_ret) * (1.0 - cost) - 1.0
        rows.append(
            {
                "date": date,
                "return": day_ret,
...
def plot_nav(nav: pd.Series) -> None:
    ...
    plt.plot(nav.index, nav.values, linewidth=1.8, label="Industry ETF Neutral Bias")
    plt.title("Industry ETF Neutral Bias Strategy NAV")
    plt.legend()
    plt.grid(alpha=0.25)
    plt.tight_layout()
...
def build_summary(result: pd.DataFrame, latest_signal: pd.DataFrame, close_df: pd.DataFrame, name_map: dict[str, str]) -> dict:
    ...
    return {
        "strategy": "industry_etf_neutral_bias",
...
def write_md(summary: dict, latest_signal: pd.DataFrame) -> None:
    ...
    for _, row in latest_signal.head(10).iterrows():
        ...
        top_rows.append(f"| {row['name']}{marker} | {row['secid']} | {row['score']:+.3f} |")
    yearly_rows = []
...
def main() -> None:
    ...
    result, score_df = run_rotation_market_neutral(mod, close_df, name_map, SCORE_MODE)
    result[["return", "nav", "holding", "next_target", "is_signal", "score"]].to_csv(
