...
# v5.8: 乖离动量 + R²过滤 + 国债ETF (替代v5.4双动量+MA拐头+冷却期)
CN_BIAS_N = 60           # 均线周期 (price / MA60)
CN_MOM_DAY = 20          # 斜率拟合窗口
CN_R2_WINDOW = 20        # R²滚动窗口
CN_R2_THRESHOLD = 0.3    # R²最低门槛
CN_BOND_CODE = "1.511260"  # 10年期国债ETF（避险资产）
CN_BOND_NAME = "10Y国债ETF"
# v5.8: 波动率缩放参数
CN_TARGET_VOL = 0.15          # 目标年化波动率
...
CN_EQUITY_CODES = ["1.515100", "0.159915", "1.000016", "1.000852", "1.000905"]
CN_ALL_CODES = CN_EQUITY_CODES + [CN_BOND_CODE]
# DK策略仍需沪深300从cn_close读取, 所以fetch列表包含它
...
# 10年期国债ETF拼接 (511260上市前用10年期国债指数)
CN_BOND_ETF_SECID = "1.511260"
CN_BOND_ETF_START = pd.Timestamp("2017-08-25")
...
# v5.8: 多配对Top-1 + 乖离动量 + VolScaling (替代v5.4单配对+冷却期)
CN_DK_COLS = ["DK_ZZ1000", "DK_SZ50"]  # cn_dk_close 列名(保留用于数据获取)
...
# v5.8 多配对索引: 5指数 → C(5,2)=10配对
CN_DK_INDICES = {
    'SZ50':   {'col': 'DK_SZ50',   'src': 'dk'},     # from cn_dk_close
    'HS300':  {'col': '1.000300',   'src': 'cn'},     # from cn_close
    'ZZ500':  {'col': '1.000905',   'src': 'cn'},
    'ZZ1000': {'col': 'DK_ZZ1000', 'src': 'dk'},
    'CYB':    {'col': '0.159915',   'src': 'cn'},
}
CN_DK_INDEX_NAMES = {
...
# 调仓阈值（参与交易决策）
US_ROT_REBALANCE_THRESHOLD = 1.0  # v5.8: 移除调仓阈值(=1.0等于无阈值)
...
PROD_VS_VOL_WINDOW = 20          # 已实现波动率回看窗口(交易日)
PROD_VS_MAX_LEV = 1.5            # 最大杠杆倍数
PROD_VS_MIN_LEV = 0.5            # 最小仓位比例
PROD_VS_THRESHOLD = 0.05         # scale变动阈值 (Δscale ≥ 5% 才调整)
...
def run_cn_strategy(close_df, equity_codes):
    """Sub-A V5.8: 乖离动量 + R²过滤 + 国债ETF轮动 + 波动率缩放.
    v5.8变更: 乖离动量替代双动量排名, R²过滤替代MA拐头和冷却期, 国债ETF加入轮动池, 波动率缩放控制风险.
    """
    ...
    # 波动率缩放 (v5.8): cash日scale=1.0, 权益日scale=target_vol/realized_vol
...
def _run_single_pair_dk(a_prices, b_prices):
    ...
    if CN_DK_SCALE_THRESHOLD > 0:
        ...
        for _i in range(len(_sa)):
            if np.isnan(_sa[_i]): continue
            if np.isnan(_last): _last = _sa[_i]
            elif abs(_sa[_i] - _last) >= CN_DK_SCALE_THRESHOLD: _last = _sa[_i]
            else: _sa[_i] = _last
        d['scale'] = _sa
...
def run_dk_strategy(cn_close, cn_dk_close):
    """Sub-A-DK V5.8: 多配对Top-1 + 乖离动量 + VolScaling.
    v5.8变更: 多配对Top-1替代单配对, 乖离动量替代绝对动量, 移除冷却期, 移除NAV MA过滤, 移除R²过滤.
    ...
    result = pd.DataFrame({
        ...
        'top_pair': top_pair_list,
        'direction': top_dir_list,
...
class CombinedStrategyBase:
    ...
    def _fetch_data(self, msg):
        ...
        try:
            # ZZHL三层拼接: H00922(H30269上市前) + H30269(ETF上市前) + ETF
            zzhl_h30269, zzhl_h00922 = None, None
            for src_name, fetcher in [
                ...
                try:
                    ...
                    time.sleep(0.5)
            try:
                df = _fetch_cn_csindex("H00922")
                if df is not None and len(df) > 50:
                    zzhl_h00922 = df
            except _DATA_FETCH_ERRORS:
                pass
            if zzhl_h30269 is None and zzhl_h00922 is None:
                raise ValueError("所有ZZHL数据源均失败")
            zzhl_etf_df = cn_raw[CN_ZZHL_ETF_SECID]
            zzhl_parts = []
            if zzhl_h30269 is not None:
                h30269_pre = zzhl_h30269[zzhl_h30269.index < CN_ZZHL_ETF_START].copy()
                if len(h30269_pre) > 0:
                    h30269_pre["close"] *= zzhl_etf_df["close"].iloc[0] / h30269_pre["close"].iloc[-1]
                    zzhl_parts.append(h30269_pre)
                    msg.write(f"  ZZHL: H30269 {h30269_pre.index[0].strftime('%Y-%m-%d')}~{CN_ZZHL_ETF_START.strftime('%Y-%m-%d')}\n")
                    if zzhl_h00922 is not None:
                        h30269_start = h30269_pre.index[0]
                        h00922_pre = zzhl_h00922[zzhl_h00922.index < h30269_start].copy()
                        if len(h00922_pre) > 0:
                            h00922_pre["close"] *= h30269_pre["close"].iloc[0] / h00922_pre["close"].iloc[-1]
                            zzhl_parts.insert(0, h00922_pre)
                            msg.write(f"  ZZHL: H00922 {h00922_pre.index[0].strftime('%Y-%m-%d')}~{h30269_start.strftime('%Y-%m-%d')}\n")
            elif zzhl_h00922 is not None:
                h00922_pre = zzhl_h00922[zzhl_h00922.index < CN_ZZHL_ETF_START].copy()
                if len(h00922_pre) > 0:
                    h00922_pre["close"] *= zzhl_etf_df["close"].iloc[0] / h00922_pre["close"].iloc[-1]
                    zzhl_parts.append(h00922_pre)
                    msg.write(f"  ZZHL: H00922 {h00922_pre.index[0].strftime('%Y-%m-%d')}~{CN_ZZHL_ETF_START.strftime('%Y-%m-%d')}\n")
            if zzhl_parts:
                zzhl_parts.append(zzhl_etf_df[zzhl_etf_df.index >= CN_ZZHL_ETF_START])
                cn_raw[CN_ZZHL_ETF_SECID] = pd.concat(zzhl_parts)
                msg.write(f"  ZZHL混合: + ETF{CN_ZZHL_ETF_START.strftime('%Y-%m-%d')}~\n")
        except _DATA_FETCH_ERRORS as e:
    ...
    def _run_strategies(self, cn_close, cn_dk_close, us_rot_close, us_prod_daily):
        # v5.8: Sub-A uses bias momentum + R² + bond ETF
        ...
        # v5.8: Sub-A-DK uses multi-pair Top-1
    ...
    def _compute_signal_data(self, cn_close, cn_dk_close, us_rot_close, us_prod_daily):
        ...
        # v5.8: compute bias momentum and R² for display
        cn_close_with_bond = cn_close.copy()
        if CN_BOND_CODE not in cn_close_with_bond.columns:
            try:
                bond_df, _ = fetch_cn_kline(CN_BOND_CODE)
        ...
        # v5.8: Multi-pair DK - extract top pair and direction
        ...
        is_dk_signal = True  # v5.8: daily signal, always active
        hypo_dk = dk_current
        ...
        if PROD_VS_ENABLED:
            ...
            subc_vs_info = {
                ...
                "target_scale": float(_vs_target_scale.iloc[-1]) if len(_vs_target_scale) > 0 else 1.0,
...
poe.update_settings(SettingsResponse(
    introduction_message=(
        ...
        "**📊 仓位管理:** \"设置仓位 Sub-B: QQQM 100股 GLDM 50股\" 或 \"设置仓位 Sub-A-DK: 中证1000持仓200万\" -> 信号自动显示调整建议\n"
    ),
))

class CombinedStrategyV58(CombinedStrategyBase):
        ...
        if not PROD_USE_TIMING:
            ...
            if PROD_VS_ENABLED:
                ...
                if abs(_vs_ts - _vs_scale) > 0.001:
                    msg.write(f" (Δ={abs(_vs_ts - _vs_scale):.2f} < {PROD_VS_THRESHOLD:.0%}阈值，未调整)")
                msg.write("\n")
                if _vs_scale > 1.0:
    ...
    def _handle_signal(self):
        ...
        with _sm() as msg:
            ...
            if "weight" in cn_result.columns and len(cn_result) >= 2:
                ...
                if abs(_cn_sc_rt - _cn_sc_prev_rt) > 0.001:
                    w(f"📊 杠杆调整: {_cn_sc_prev_rt:.2f}x -> **{_cn_sc_rt:.2f}x**")
                else:
                    w(f"📊 杠杆维持: **{_cn_sc_rt:.2f}x**")
                if CN_SCALE_THRESHOLD > 0 and not np.isnan(_cn_sc_raw_rt) and abs(_cn_sc_raw_rt - _cn_sc_rt) > 0.001:
                    w(f" | 理论: {_cn_sc_raw_rt:.2f}x (|Δ|={abs(_cn_sc_raw_rt - _cn_sc_rt):.2f} < {CN_SCALE_THRESHOLD}阈值，未调整)")
                w("\n")
            # v5.8: 乖离动量 + R² 排名表
            ...
            if _sorted_live:
                ...
                if not np.isnan(_best_bm) and _best_bm <= 0:
                ...
                elif _best == CN_BOND_CODE:
                    w(f"\n**选择:** 乖离动量最高 -> **{_best_name}** (国债ETF免R²过滤)\n")
                else:
                    ...
                    w(f"**R²过滤:** R²({CN_R2_WINDOW})={_r2v_best:.3f} -> {'**通过** ✅' if _r2_pass else '**未通过** ❌ -> 选次优或国债ETF'}\n")
            # 成交量情绪（仅展示）
            ...
            if cn_open and cn_data_is_today:
                w(" ⚡盘中实时")
            w("\n")
            # v5.8: Multi-pair position display
            ...
            if "weight" in cn_dk_result.columns and len(cn_dk_result) >= 2:
                ...
                if abs(_dk_sc_rt - _dk_sc_prev_rt) > 0.001:
                    w(f"📊 杠杆调整: {_dk_sc_prev_rt:.2f}x -> **{_dk_sc_rt:.2f}x**")
                else:
                    w(f"📊 杠杆维持: **{_dk_sc_rt:.2f}x**")
                if CN_DK_SCALE_THRESHOLD > 0 and abs(_dk_sc_raw_rt - _dk_sc_rt) > 0.001:
                    w(f" | 理论: {_dk_sc_raw_rt:.2f}x (|Δ|={abs(_dk_sc_raw_rt - _dk_sc_rt):.2f} < {CN_DK_SCALE_THRESHOLD}阈值，未调整)")
                w("\n")
            w("\n---\n\n")
            us_close_bj = beijing_time_str(us_date, "US", "close")
            w("### Sub-B: 美股7ETF\n")
            ...
            for etf in _us_all_etfs:
                if etf in us_rot_close.columns:
                    _us_latest_prices[_ROT_PROXY_TO_LIVE.get(etf, etf)] = \
                        us_rot_close[etf].iloc[-1]
            if _sub_b_capital:
            ...
            if _sub_b_pos:
                ...
                _target_val = _sub_b_capital if _sub_b_capital else _total_cur_val
                if _target_val and _target_val > 0:
                    w(f"\n📊 **仓位调整** (基于已设置的当前持仓, 总值${_target_val:,.0f}):\n")
                    w("| ETF | 当前持仓 | 目标数量 | 调整 |\n|:-|--------:|--------:|-----:|\n")
    ...
    def _handle_live_signal(self):
        ...
        turnover_b = d["turnover_b"]
        all_a = d["all_a"]
        # v5.8: bias momentum + R² display data
        ...
        with _sm() as msg:
            ...
            if "weight" in cn_result.columns and len(cn_result) >= 2:
                ...
                w(f" | 目标: {CN_TARGET_VOL:.0%}\n")
                if abs(_cn_sc_rt3 - _cn_sc_prev_rt3) > 0.001:
                    w(f"📊 杠杆调整: {_cn_sc_prev_rt3:.2f}x -> **{_cn_sc_rt3:.2f}x**")
                else:
                    w(f"📊 杠杆维持: **{_cn_sc_rt3:.2f}x**")
                if CN_SCALE_THRESHOLD > 0 and not np.isnan(_cn_sc_raw_rt3) and abs(_cn_sc_raw_rt3 - _cn_sc_rt3) > 0.001:
                    w(f" | 理论: {_cn_sc_raw_rt3:.2f}x (|Δ|={abs(_cn_sc_raw_rt3 - _cn_sc_rt3):.2f} < {CN_SCALE_THRESHOLD}阈值，未调整)")
                w("\n")
            # v5.8: Bias momentum + R² ranking (detailed view)
            ...
            w(f"**乖离动量排名** (v5.8: bias_mom + R² filter):\n\n")
            ...
            if _sorted_live2:
                ...
                if not np.isnan(_best2_bm) and _best2_bm <= 0:
                ...
                elif _best2 == CN_BOND_CODE:
                    w(f"\n**选择:** 乖离动量最高 -> **{_best2_name}** (国债ETF免R²过滤)\n")
                else:
                    ...
                    w(f"**R²过滤:** R²({CN_R2_WINDOW})={_r2v_best2:.3f} -> {'**通过** ✅' if _r2_pass2 else '**未通过** ❌ -> 选次优或国债ETF'}\n")
                # 成交量情绪（仅展示）
            ...
            if "weight" in cn_dk_result.columns and len(cn_dk_result) >= 2:
                ...
                w(f" | 目标: {CN_DK_TARGET_VOL:.0%}\n")
                if abs(_dk_sc_rt3 - _dk_sc_prev_rt3) > 0.001:
                    w(f"📊 杠杆调整: {_dk_sc_prev_rt3:.2f}x -> **{_dk_sc_rt3:.2f}x**")
                else:
                    w(f"📊 杠杆维持: **{_dk_sc_rt3:.2f}x**")
                if CN_DK_SCALE_THRESHOLD > 0 and abs(_dk_sc_raw_rt3 - _dk_sc_rt3) > 0.001:
                    w(f" | 理论: {_dk_sc_raw_rt3:.2f}x (|Δ|={abs(_dk_sc_raw_rt3 - _dk_sc_rt3):.2f} < {CN_DK_SCALE_THRESHOLD}阈值，未调整)")
                w("\n")
            w("\n---\n\n")
            us_close_bj = beijing_time_str(us_date, "US", "close")
            w("### Sub-B: 美股7ETF\n")
            ...
            if _sub_b_pos_live:
                ...
                for etf in all_a:
                    if etf in us_rot_close.columns:
                        _us_live_prices[_ROT_PROXY_TO_LIVE.get(etf, etf)] = us_rot_close[etf].iloc[-1]
                ...
                _target_val_live = _sub_b_cap_live if _sub_b_cap_live else _total_cur_val_live
                if _target_val_live and _target_val_live > 0:
                    w(f"\n📊 **仓位调整** (基于当前持仓, 总值${_target_val_live:,.0f}):\n")
                    w("| ETF | 当前持仓 | 目标数量 | 调整 |\n|:-|--------:|--------:|-----:|\n")
    ...
    def _handle_params(self):
        with _sm() as msg:
            ...
            w(f"| R²门槛 | **{CN_R2_THRESHOLD}** | 权益ETF需R²≥{CN_R2_THRESHOLD}才持有 |\n")
            w(f"| 国债ETF | **{CN_BOND_NAME}({CN_BOND_CODE})** | 避险资产，免R²过滤 |\n")
            ...
            w(f"| 冷却期 | **无** | v5.8移除（乖离动量信号天然平滑） |\n")
            w("\n**计算过程:**\n")
            w(f"1. 乖离率: `bias = price / MA({CN_BIAS_N})`\n")
            w(f"2. 乖离动量: 最近{CN_MOM_DAY}日bias归一化后线性拟合斜率×10000\n")
            w("3. 选乖离动量最高的资产\n")
            w(f"4. 权益ETF需R²({CN_R2_WINDOW})≥{CN_R2_THRESHOLD}才持有，国债ETF免R²过滤\n")
            ...
            w(f"| 调仓阈值 | **{US_ROT_REBALANCE_THRESHOLD}x** | v5.8: 移除阈值(=1.0等于无阈值) |\n")
    ...
    def _handle_live_params(self):
        ...
        with _sm() as msg:
            ...
            _r2_latest_lp = {c: float(r2_lp[c].iloc[-1]) for c in CN_EQUITY_CODES if c in r2_lp and not np.isnan(r2_lp[c].iloc[-1])}
            ...
            if best_code:
                ...
                elif best_code == CN_BOND_CODE:
                    w(f"\n**② 选股:** 乖离动量最高 -> **{best_name}** (国债ETF免R²过滤)\n")
                else:
            ...
            if "weight" in cn_result.columns and len(cn_result) >= 2:
                ...
                if CN_SCALE_THRESHOLD > 0:
                    if not np.isnan(_cn_sc_raw_p) and abs(_cn_sc_raw_p - _cn_sc_p) > 0.001:
                        w(f"| 理论杠杆 | {_cn_sc_raw_p:.2f}x (|Δ|={abs(_cn_sc_raw_p - _cn_sc_p):.2f} < {CN_SCALE_THRESHOLD}阈值) |\n")
                    else:
                        w(f"| 调整阈值 | Δ≥{CN_SCALE_THRESHOLD:.2f} |\n")
                if abs(_cn_sc_p - _cn_sc_prev_p) > 0.001:
                    w(f"\n📊 杠杆调整: {_cn_sc_prev_p:.2f}x -> **{_cn_sc_p:.2f}x**\n")
                else:
            ...
            if "weight" in cn_dk_result.columns and len(cn_dk_result) >= 2:
                ...
                if CN_DK_SCALE_THRESHOLD > 0:
                    if abs(_dk_sc_raw_p - _dk_sc_p) > 0.001:
                        w(f"| 理论杠杆 | {_dk_sc_raw_p:.2f}x (|Δ|={abs(_dk_sc_raw_p - _dk_sc_p):.2f} < {CN_DK_SCALE_THRESHOLD}阈值) |\n")
                    else:
                        w(f"| 调整阈值 | Δ≥{CN_DK_SCALE_THRESHOLD:.2f} |\n")
                if abs(_dk_sc_p - _dk_sc_prev_p) > 0.001:
                    w(f"\n📊 杠杆调整: {_dk_sc_prev_p:.2f}x -> **{_dk_sc_p:.2f}x**\n")
                else:
            ...
            if PROD_VS_ENABLED:
                ...
                _c_ts_now = float(_vs_target_scale_p.iloc[-1]) if len(_vs_target_scale_p) > 0 else 1.0
                ...
                if abs(_c_ts_now - _c_scale_now) > 0.001:
                    w(f"| 阈值状态 | |Δ|={abs(_c_ts_now - _c_scale_now):.2f} < {PROD_VS_THRESHOLD}阈值，未调整 |\n")
                w(f"| 杠杆范围 | [{PROD_VS_MIN_LEV:.1f}x, {PROD_VS_MAX_LEV:.1f}x] |\n")
                w(f"| 观察窗口 | {PROD_VS_VOL_WINDOW}d |\n")
                w(f"| 阈值 | Δ≥{PROD_VS_THRESHOLD:.0%} |\n")
                if abs(_c_scale_now - _c_scale_prev) > 0.001:
                    w(f"\n📊 杠杆调整: {_c_scale_prev:.2f}x -> **{_c_scale_now:.2f}x**\n")
                else:
    ...
    def _handle_signal_history(self, query):
        ...
        cn_result, cn_dk_result, us_rot_result, *_ = \
            self._run_strategies(cn_close, cn_dk_close, us_rot_close, us_prod_daily)
        # ---- HS300 MA series for Sub-A ----
        hs300_col = "1.000300"
        # v5.8: No MA filter, using placeholder for history display
