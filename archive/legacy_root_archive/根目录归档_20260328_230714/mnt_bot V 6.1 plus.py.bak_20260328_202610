...
def _build_dk_rank_rows(cn_dk_result, use_shifted=True, top_n=3):
    ...
    for rank, (pair, score_used) in enumerate(row.items(), 1):
        ...
        rows.append({
            "rank": rank,
            "pair": pair,
            "pair_display": "/".join(_dk_leg_name(p) for p in str(pair).split("/")) if pair != "none" else "none",
...
class CombinedStrategyV61(CombinedStrategyBase):
    ...
    def _handle_signal(self):
        ...
        with _sm() as msg:
            ...
            if dk_pair_changed or dk_direction_changed:
                _dk_change_bits = []
                if dk_pair_changed:
                    _dk_change_bits.append("配对切换")
                if dk_direction_changed:
                    _dk_change_bits.append("方向切换")
                w(f"**今日变化:** {' + '.join(_dk_change_bits)}\n")
            if dk_rank_current:
                w("**当前已生效Top-3配对:**\n")
                for row in dk_rank_current:
                    _current_mark = " ← 当前" if row["pair"] == dk_top_pair else ""
                    _score_used = row["score_used"]
                    _score_live = row["score_live"]
                    _score_used_s = f"{_score_used:.2f}" if not np.isnan(_score_used) else "NA"
                    _score_live_s = f"{_score_live:.2f}" if not np.isnan(_score_live) else "NA"
                    w(f"- {row['rank']}. **{row['pair_display']}** | 生效分数 `{_score_used_s}` | 今日分数 `{_score_live_s}` | {row['position_text']}{_current_mark}\n")
            if dk_rank_today:
                w(f"**今日假设信号:** **{dk_hypo_top_pair}** | 方向 {dk_hypo_direction:+d} | {hypo_dk_name}\n")
            # ── DK vol-scaling 杠杆显示 ──
    ...
    def _handle_live_signal(self):
        ...
        with _sm() as msg:
            ...
            if dk_pair_changed or dk_direction_changed:
                _dk_change_bits3 = []
                if dk_pair_changed:
                    _dk_change_bits3.append("配对切换")
                if dk_direction_changed:
                    _dk_change_bits3.append("方向切换")
                w(f"**今日变化:** {' + '.join(_dk_change_bits3)}\n")
            if dk_rank_current:
                w("**当前已生效Top-3配对:**\n")
                for row in dk_rank_current:
                    _current_mark3 = " ← 当前" if row["pair"] == dk_top_pair else ""
                    _score_used3 = row["score_used"]
                    _score_live3 = row["score_live"]
                    _score_used_s3 = f"{_score_used3:.2f}" if not np.isnan(_score_used3) else "NA"
                    _score_live_s3 = f"{_score_live3:.2f}" if not np.isnan(_score_live3) else "NA"
                    w(f"- {row['rank']}. **{row['pair_display']}** | 生效分数 `{_score_used_s3}` | 今日分数 `{_score_live_s3}` | {row['position_text']}{_current_mark3}\n")
            if dk_rank_today:
                w(f"**今日假设信号:** **{dk_hypo_top_pair}** | 方向 {dk_hypo_direction:+d} | {_dk_pos_str(hypo_dk)}\n")
