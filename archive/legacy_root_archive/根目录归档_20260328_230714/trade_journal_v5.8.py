...
CN_STOCK_CODES = ["1.515100", "0.159915", "1.000016", "1.000852", "1.000905", "1.000300", "1.511260"]
# 上证50替换沪深300入Sub-A轮动池; 沪深300保留供DK使用
...
CN_HS300_SECID = "1.000300"  # 沪深300指数代码 (DK仍使用)
...
CN_DK_INDICES = {
    'SZ50': 'DK_SZ50', 'HS300': '1.000300', 'ZZ500': '1.000905',
    'ZZ1000': 'DK_ZZ1000', 'CYB': '0.159915',
}
CN_DK_INDEX_NAMES = {
...
def run_cn_strategy(close_df, ranking_codes):
    """Sub-A V5.8: 乖离动量 + R²过滤 + 国债ETF + 波动率缩放, 无冷却期."""
...
def _dk_run_single_pair(a_prices, b_prices):
    """对单个配对运行乖离动量策略, 返回 (strategy_ret, abs_bias_mom) 或 (None, None)"""
    ...
    if CN_DK_SCALE_THRESHOLD > 0:
        ...
        for _i in range(len(_sa)):
            ...
            if np.isnan(_last):
                _last = _sa[_i]
            elif abs(_sa[_i] - _last) >= CN_DK_SCALE_THRESHOLD:
                _last = _sa[_i]
            else:
                _sa[_i] = _last
        d['scale'] = _sa
...
def _dk_build_top_n(rets_df, signals_df, n=CN_DK_TOP_N):
    ...
    for date in rets_df.index:
        ...
        active_pair[date] = selected[0] if len(selected) > 0 else ""
    return portfolio_ret, active_pair

def run_dk_strategy(cn_close, cn_dk_close):
    """Sub-A-DK V5.8: 多配对Top-1 + 二值R² + 波动率缩放, 无冷却期.
    5指数(SZ50,HS300,ZZ500,ZZ1000,CYB) → C(5,2)=10配对
    每对独立运行乖离动量策略, 每天选信号最强的1对
    """
    from itertools import combinations
    indices = {}
    for name, col in CN_DK_INDICES.items():
        if col.startswith("DK_"):
            if col in cn_dk_close.columns:
                indices[name] = cn_dk_close[col]
        else:
            if col in cn_close.columns:
                indices[name] = cn_close[col]
    ...
    for a_name, b_name in pairs_all:
        ...
        ret, sig = _dk_run_single_pair(indices[a_name], indices[b_name])
    ...
    t_ret, active_pair = _dk_build_top_n(rets_df, sigs_df, CN_DK_TOP_N)
    result = pd.DataFrame({'return': t_ret, 'active_pair': active_pair})
...
class CombinedStrategyBase:
    ...
    def _fetch_data(self, msg):
        ...
        # 国债ETF拼接: 511260上市前用10年期国债指数
        if CN_BOND_ETF_SECID in cn_raw:
            try:
                bond_idx_df, bond_idx_src = None, None
                for src_name, fetcher in [
                    ("csindex 931643", lambda: _fetch_cn_csindex("931643")),
                    ("EastMoney 1.000269", lambda: _fetch_cn_eastmoney("1.000269")),
                    ("csindex H00269", lambda: _fetch_cn_csindex("H00269")),
                ]:
                    try:
                        df = fetcher()
                        if df is not None and len(df) > 50:
                            bond_idx_df, bond_idx_src = df, src_name
                            break
                    except _DATA_FETCH_ERRORS:
                        time.sleep(0.5)
                if bond_idx_df is not None:
                    bond_etf_df = cn_raw[CN_BOND_ETF_SECID]
                    bond_before = bond_idx_df[bond_idx_df.index < CN_BOND_ETF_START]
                    bond_after = bond_etf_df[bond_etf_df.index >= CN_BOND_ETF_START]
                    if len(bond_before) > 0 and len(bond_after) > 0:
                        b_idx_last = bond_before["close"].iloc[-1]
                        b_etf_first = bond_after["close"].iloc[0]
                        b_scale = b_etf_first / b_idx_last
                        bond_before = bond_before.copy()
                        bond_before["close"] = bond_before["close"] * b_scale
                        cn_raw[CN_BOND_ETF_SECID] = pd.concat([bond_before, bond_after])
                        msg.write(f"  国债混合: 指数{bond_idx_df.index[0].strftime('%Y-%m-%d')}~{CN_BOND_ETF_START.strftime('%Y-%m-%d')}"
                                 f" + ETF{CN_BOND_ETF_START.strftime('%Y-%m-%d')}~ [{bond_idx_src}+ETF]\n")
            except _DATA_FETCH_ERRORS as e:
                msg.write(f"  ⚠️ 国债指数拼接失败({e})，仅用511260 ETF数据\n")
        # 合并: 权益标的先合并, 国债ETF左连接 (国债历史较短, 不影响权益起始日期)
        equity_codes = [c for c in CN_STOCK_CODES if c != CN_BOND_CODE]
        cn_close = pd.concat([cn_raw[s].rename(columns={"close": s})
                              for s in equity_codes], axis=1).ffill().dropna()
        if CN_BOND_CODE in cn_raw:
            bond_col = cn_raw[CN_BOND_CODE].rename(columns={"close": CN_BOND_CODE})
            cn_close = cn_close.join(bond_col, how='left')
        ...
        msg.write("⏳ 正在获取A-DK多配对数据(中证1000/上证50)...\n")
        try:
            dk_dfs = {}
    ...
    def _compute_signal_data(self, cn_close, cn_dk_close, us_rot_close, us_prod_daily):
        ...
        # Sub-A v5.8: bias momentum + R² (no cooldown, no MA filter)
        bias_scores = {}
        r2_scores = {}
        for code in CN_STOCK_CODES:
            bias_scores[code] = _calc_bias_momentum(cn_close[code]).iloc[-1]
            if code != CN_BOND_CODE:
                r2_scores[code] = _calc_rolling_r2(cn_close[code]).iloc[-1]
        hypo_cn = "cash"
        valid_scores = {k: v for k, v in bias_scores.items() if not np.isnan(v)}
        if valid_scores:
            best_cn = max(valid_scores, key=valid_scores.get)
            if valid_scores[best_cn] <= 0:
                hypo_cn = "cash"  # 乖离动量全负 → 持现金
            elif best_cn == CN_BOND_CODE:
                hypo_cn = best_cn
            else:
                r2_val = r2_scores.get(best_cn, np.nan)
                if not np.isnan(r2_val) and r2_val >= CN_R2_THRESHOLD:
                    hypo_cn = best_cn
        us_date = us_rot_close.index[-1]
        ...
        # Sub-A-DK v5.8: 多配对Top-1, active_pair列记录当前最强配对
        dk_active_pair = cn_dk_result["active_pair"].iloc[-1] if "active_pair" in cn_dk_result.columns else ""
        dk_current = dk_active_pair  # 用于持仓显示
        ...
        return {
            ...
            "hypo_cn": hypo_cn,
            "bias_scores": bias_scores, "r2_scores": r2_scores,
            "dk_date": dk_date,
            "dk_current": dk_current, "dk_active_pair": dk_active_pair,
...
poe.update_settings(SettingsResponse(
    introduction_message=(
        ...
        "**📊 仓位管理:** \"设置仓位 Sub-B: QQQM 100股 GLDM 50股\" 或 \"设置仓位 Sub-A-DK: 中证1000持仓200万\"\n"
    ),
))
...
14. "红利低波"="ZZHL-ETF"=1.515100, "创业板"="CYB-ETF"=0.159915"""
    ...
    def _handle_live_portfolio(self):
        ...
        with _sm() as msg:
            ...
            # Sub-A vol-scaling 杠杆
            _cn_res_j = d.get("cn_result")
            if _cn_res_j is not None and "weight" in _cn_res_j.columns and len(_cn_res_j) >= 1:
                _cn_sc_j = _cn_res_j["weight"].iloc[-1]
                _cn_rv_j = _cn_res_j["realized_vol"].iloc[-1] if "realized_vol" in _cn_res_j.columns else None
                _sc_str = f"scale={_cn_sc_j:.2f}x"
                if _cn_rv_j is not None and not np.isnan(_cn_rv_j):
                    _sc_str += f" | vol={_cn_rv_j:.1%}"
                w(f"| Sub-A杠杆 | {_sc_str} | 目标{CN_TARGET_VOL:.0%} | — |\n")
            dk_rec = latest.get("Sub-A-DK")
            actual_dk = dk_rec.get("to_holding", "—") if dk_rec else "—"
            dk_pair = d.get("dk_active_pair", "")
            theo_dk = f"Top-1: {dk_pair}" if dk_pair else "—"
            if dk_rec:
                match_dk = "📊"
            else:
                match_dk = "未记录"
