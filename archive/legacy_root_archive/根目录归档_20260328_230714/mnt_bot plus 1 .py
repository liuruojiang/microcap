...
PROD_VS_VOL_WINDOW = 15          # 已实现波动率回看窗口(交易日)  # V5.8采用v6.1试验参数: 20→15
PROD_VS_MAX_LEV = 1.5            # 最大杠杆倍数
PROD_VS_MIN_LEV = 0.5            # 最小仓位比例
PROD_VS_THRESHOLD = 0.10         # scale变动阈值 (Δscale ≥ 10% 才调整)  # V5.8采用v6.1试验参数: 5%→10%
...
def _obsolete_fetch_cn_h_proxy(secid):
...
def _obsolete_fetch_cn_kline_v2(secid):
    code = secid.split('.')[1]
    last_err = None
    attempts = []
    if code.startswith('H'):
