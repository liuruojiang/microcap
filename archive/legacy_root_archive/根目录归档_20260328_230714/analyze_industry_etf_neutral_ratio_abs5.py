...
OUT_CLOSE_CSV = ROOT / "industry_etf_neutral_ratio_abs5_close.csv"
OUT_NAV_CSV = ROOT / "industry_etf_neutral_ratio_abs5_nav.csv"
OUT_SIGNAL_CSV = ROOT / "industry_etf_neutral_ratio_abs5_latest_signal.csv"
OUT_SUMMARY_JSON = ROOT / "industry_etf_neutral_ratio_abs5_summary.json"
OUT_PNG = ROOT / "industry_etf_neutral_ratio_abs5_curve.png"
OUT_MD = ROOT / "mnt_industry_etf_neutral_ratio_abs5_20260328.md"
...
def load_main_module():
    ...
    spec = importlib.util.spec_from_file_location("mntbot_v61_ratio_abs5", MAIN_SCRIPT)
...
def plot_nav(nav: pd.Series) -> None:
    ...
    plt.plot(nav.index, nav.values, linewidth=1.8, label="Industry ETF Neutral Ratio Abs5")
    plt.title("Industry ETF Neutral Ratio Abs5 Strategy NAV")
    plt.legend()
    plt.grid(alpha=0.25)
    plt.tight_layout()
...
def build_summary(
    ...
    return {
        "strategy": "industry_etf_neutral_ratio_abs5",
        "signal_mode": "5d absolute momentum on ETF / 中证500 ratio",
...
def main() -> None:
    ...
    score_df = build_signal_frame(close_df)
    ...
    result = run_rotation_market_neutral(close_df, score_df, name_map)
