...
def build_conditional_top_pair(signals_df, *, threshold, forced_rank):
    ...
    for _, row in shifted.iterrows():
        ...
        best_pair = row.index[0]
        best_score = float(row.iloc[0])
        ...
        cur_rank = int(row.index.get_loc(current)) + 1

        if cur_score <= 0:
            current = best_pair
        elif cur_rank > forced_rank:
            current = best_pair
...
def build_dk_result_conditional(base_dk_result, *, threshold, forced_rank):
    signals_df = base_dk_result.attrs["signals_df"]
    pair_data = base_dk_result.attrs["pair_data"]
    common_idx = base_dk_result.index
    top_pair_series = build_conditional_top_pair(
        signals_df.reindex(common_idx),
        threshold=threshold,
    ...
    result.attrs["pair_switch_mode"] = {"threshold": threshold, "forced_rank": forced_rank}
    return result
...
def main():
    ...
    dk_ret_105 = adk_ret.build_dk_result_with_retention(mod, dk_base, threshold=1.05)
    ...
    output = {
        ...
        "retention_1.05x": {
            "adk": adk_ret.calc_metrics(mod, dk_ret_105.loc[(dk_ret_105.index >= start) & (dk_ret_105.index <= end), "return"]),
            "trade_frequency": trade_freq.analyze_adk(dk_ret_105, start, end),
            "pair_behavior": adk_ret.summarize_pair_behavior(dk_ret_105.loc[(dk_ret_105.index >= start) & (dk_ret_105.index <= end)]),
            "combined": lev.compute_combined_summary(
                mod,
                us_rot_result=us_rot_result,
                cn_result=cn_result,
                cn_dk_result=dk_ret_105,
                us_prod_daily=us_prod_daily,
            ),
        },
    }

    for forced_rank in [2, 3]:
        dk_alt = build_dk_result_conditional(dk_base, threshold=1.05, forced_rank=forced_rank)
        output[f"conditional_rank{forced_rank}"] = {
            "adk": adk_ret.calc_metrics(mod, dk_alt.loc[(dk_alt.index >= start) & (dk_alt.index <= end), "return"]),
            "trade_frequency": trade_freq.analyze_adk(dk_alt, start, end),
            "pair_behavior": adk_ret.summarize_pair_behavior(dk_alt.loc[(dk_alt.index >= start) & (dk_alt.index <= end)]),
            "combined": lev.compute_combined_summary(
                mod,
                us_rot_result=us_rot_result,
                cn_result=cn_result,
