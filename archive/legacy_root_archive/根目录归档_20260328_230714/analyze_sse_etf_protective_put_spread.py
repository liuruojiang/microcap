...
TRADING_DAYS = 244
ROLL_DAYS = 21
PUT_COST_MARKUP = 1.05
SHORT_PUT_HAIRCUT = 0.88
...
class PutSpreadSpec:
    ...
    @property
    def label(self) -> str:
        return f"PPS_{int(self.long_put_m*100)}/{int(self.short_put_m*100)}"


def simulate(df: pd.DataFrame, spec: PutSpreadSpec) -> pd.Series:
    close = df["close"]
    qvix = df["qvix"].ffill()
    ...
    for i in range(len(roll_locs) - 1):
        ...
        k_long = s0 * spec.long_put_m
        k_short = s0 * spec.short_put_m
        long_put_0 = bs_put_price(s0, k_long, ROLL_DAYS / TRADING_DAYS, iv0, RF_ANNUAL) * PUT_COST_MARKUP
        short_put_0 = bs_put_price(s0, k_short, ROLL_DAYS / TRADING_DAYS, iv0, RF_ANNUAL) * SHORT_PUT_HAIRCUT
        net_cost = long_put_0 - short_put_0
        ...
        for j, dt in enumerate(path_idx):
            ...
            if rem > 0:
                long_put_t = bs_put_price(s1, k_long, rem, iv1, RF_ANNUAL)
                short_put_t = bs_put_price(s1, k_short, rem, iv1, RF_ANNUAL)
            else:
                long_put_t = max(k_long - s1, 0.0)
                short_put_t = max(k_short - s1, 0.0)
            overlay_value = (long_put_t - short_put_t - net_cost) / s0
            equity.loc[dt] = account * ((s1 / s0) + overlay_value)
...
def run_all() -> dict:
    specs = [
        PutSpreadSpec(0.97, 0.92),
        PutSpreadSpec(0.95, 0.90),
        PutSpreadSpec(0.95, 0.85),
        PutSpreadSpec(0.93, 0.85),
    ]
    summary = {}
...
def main():
    summary = run_all()
    out = ROOT / "sse_etf_protective_put_spread_summary.json"
