...
OUT_CLOSE_CSV = ROOT / "industry_etf_neutral_abs20_close.csv"
OUT_NAV_CSV = ROOT / "industry_etf_neutral_abs20_nav.csv"
OUT_SIGNAL_CSV = ROOT / "industry_etf_neutral_abs20_latest_signal.csv"
OUT_SUMMARY_JSON = ROOT / "industry_etf_neutral_abs20_summary.json"
OUT_PNG = ROOT / "industry_etf_neutral_abs20_curve.png"
OUT_MD = ROOT / "mnt_industry_etf_neutral_abs20_20260328.md"
...
def load_main_module():
    ...
    spec = importlib.util.spec_from_file_location("mntbot_v61_abs20", MAIN_SCRIPT)
...
def run_rotation_market_neutral(close_df: pd.DataFrame, name_map: dict[str, str]) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    for secid in etf_secids:
        score_df[secid] = calc_abs_momentum(close_df[secid], LOOKBACK)
...
def plot_nav(nav: pd.Series) -> None:
    ...
    plt.plot(nav.index, nav.values, linewidth=1.8, label="Industry ETF Neutral Abs20")
    plt.title("Industry ETF Neutral Absolute-Momentum(20d) NAV")
    plt.legend()
    plt.grid(alpha=0.25)
    plt.tight_layout()
...
def build_summary(result: pd.DataFrame, latest_signal: pd.DataFrame, close_df: pd.DataFrame, name_map: dict[str, str]) -> dict:
    ...
    return {
        "strategy": "industry_etf_neutral_abs20",
        "signal_mode": "20d absolute momentum on ETF price",
...
def write_md(summary: dict, latest_signal: pd.DataFrame) -> None:
    ...
    md = f"""# 行业ETF市场中性策略 20日绝对动量版

## 设定

- 多头池子: 行业ETF 21 只
- 对冲腿: {HEDGE_NAME} (`{HEDGE_SECID}`) 价格指数，作为中证500期货的价格代理
- 信号模式: ETF 自身 `{LOOKBACK}` 日绝对动量
- 交易规则:
  - 每天比较全部行业ETF的 `{LOOKBACK}` 日收益率
