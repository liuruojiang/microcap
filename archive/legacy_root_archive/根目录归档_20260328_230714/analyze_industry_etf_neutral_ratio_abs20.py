...
OUT_CLOSE_CSV = ROOT / "industry_etf_neutral_ratio_abs20_close.csv"
OUT_NAV_CSV = ROOT / "industry_etf_neutral_ratio_abs20_nav.csv"
OUT_SIGNAL_CSV = ROOT / "industry_etf_neutral_ratio_abs20_latest_signal.csv"
OUT_SUMMARY_JSON = ROOT / "industry_etf_neutral_ratio_abs20_summary.json"
OUT_PNG = ROOT / "industry_etf_neutral_ratio_abs20_curve.png"
OUT_MD = ROOT / "mnt_industry_etf_neutral_ratio_abs20_20260328.md"
...
def load_main_module():
    ...
    spec = importlib.util.spec_from_file_location("mntbot_v61_ratio_abs20", MAIN_SCRIPT)
...
def plot_nav(nav: pd.Series) -> None:
    ...
    plt.plot(nav.index, nav.values, linewidth=1.8, label="Industry ETF Neutral Ratio Abs20")
    plt.title("Industry ETF Neutral Ratio Absolute-Momentum(20d) NAV")
    plt.legend()
    plt.grid(alpha=0.25)
    plt.tight_layout()
...
def build_summary(result: pd.DataFrame, latest_signal: pd.DataFrame, close_df: pd.DataFrame, name_map: dict[str, str]) -> dict:
    ...
    return {
        "strategy": "industry_etf_neutral_ratio_abs20",
        "signal_mode": "20d absolute momentum on ETF / 中证500 ratio",
...
def write_md(summary: dict, latest_signal: pd.DataFrame) -> None:
    ...
    md = f"""# 行业ETF市场中性策略 Ratio-Abs20 版
...
- 交易规则:
  - 每天比较全部行业ETF相对中证500的 `{LOOKBACK}` 日超额走势
