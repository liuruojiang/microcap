...
OUTPUT_PREFIX = "microcap_top100_mom16_biweekly_live_v2_2"
...
COSTED_NAV_CSV = OUTPUT_DIR / "microcap_top100_mom16_exp_h6_lb17_gap10_decay25_recovery35_targetvol25_max1p5_v2_2_costed_nav.csv"
...
EXPECTED_VERSION_NOTE_PREFIX = "Formal v2.2 spread-NAV log-WLS target-volatility overlay."
LOOKBACK = 17
HALFLIFE = 6.0
MOMENTUM_GAP_EXIT_BUFFER = 0.10
...
def always_on_spread_nav(close_df: pd.DataFrame) -> tuple[pd.Series, pd.Series, pd.Series, float]:
    ...
    daily_drag = float(v2_0.base_mod.FUTURES_DRAG) * BASE_HEDGE_RATIO
    spread_ret = micro_ret.fillna(0.0) - BASE_HEDGE_RATIO * hedge_ret.fillna(0.0) - daily_drag
...
def build_spread_log_wls_gross(close_df: pd.DataFrame, index: pd.DatetimeIndex | None = None) -> pd.DataFrame:
    close_df = close_df.sort_index()
    spread_nav, micro_ret, hedge_ret, daily_drag = always_on_spread_nav(close_df)
    log_wls = log_wls_score_and_r2(spread_nav)
    ...
    active_spread_ret = microcap_ret.fillna(0.0) - BASE_HEDGE_RATIO * hedge_ret_part.fillna(0.0)
    futures_drag = pd.Series(
        np.where(current_active, daily_drag, 0.0),
        index=common_index,
        dtype=float,
    )
...
def current_base_fingerprint() -> dict[str, object]:
    ...
    return {
        ...
        "signal_model": "spread_nav_log_wls_exp_halflife_6p0_lb17",
        "lookback": LOOKBACK,
        "halflife": HALFLIFE,
        "exp_weight_oldest_to_newest": list(exp_weights()),
        "common_index_source": "intersection of valid spread-NAV log-WLS signal dates and official v2.0 output index",
        "score_definition": "annualized weighted log slope of always-on 0.8x hedged spread NAV",
        "r2_gate": None,
...
def summary_matches_current_v2_2_base(summary: dict[str, object]) -> bool:
...
def incompatible_v2_2_outputs() -> list[Path]:
    outputs = [
        SUMMARY_JSON,
        LATEST_SIGNAL_CSV,
        REALTIME_SIGNAL_CSV,
    ...
    try:
        ...
        summary = None
    if summary_matches_current_v2_2_base(summary):
        return []
    return outputs
...
def build_performance_payload(ret: pd.Series, source_label: str = "costed_v2_2") -> dict[str, object]:
    ensure_output_dir()
    ...
    plt.plot(nav_df["date"], nav_df["nav_net"], label="v2.2 nav_net")
    plt.title("Top100 Microcap Mom16 v2.2 Costed NAV")
    plt.xlabel("date")
    plt.ylabel("nav_net")
...
def _build_signal_row(net_df: pd.DataFrame, reference_summary: dict[str, object]) -> pd.DataFrame:
    ...
    row["signal_model"] = "spread_nav_log_wls_exp_halflife_6p0_lb17"
...
def generate_v2_2_outputs() -> tuple[dict[str, object], pd.DataFrame, pd.DataFrame]:
    ensure_output_dir()
    ...
    stale_outputs = incompatible_v2_2_outputs()
    ...
    out = build_v2_2_result(close_df, turnover_df, common_index)
    if COSTED_NAV_CSV.exists() and COSTED_NAV_CSV not in stale_outputs:
        ...
        v2_0.base_mod.assert_no_historical_rewrite(
            ...
            label="v2.2 official costed NAV",
    ...
    perf_payload = build_performance_payload(out["return_net"].fillna(0.0), source_label="costed_v2_2")
    ...
    summary["version_note"] = (
        "Formal v2.2 spread-NAV log-WLS target-volatility overlay. Uses exp half-life 6.0 weighted log slope on "
        "17 trading days of always-on 0.8x hedged spread NAV, no R2 gate, 10% score exit buffer, peak-decay derisk "
        "with decay/recovery thresholds 0.25/0.35 and derisk scale 0.0, 60-day realized volatility, 25% annual "
        "target volatility, max 1.5x leverage, 10bp leg-turnover scale-change cost, scaled embedded-lineage base "
    ...
    summary["performance_source_label"] = "costed_v2_2"
...
def build_realtime_v2_2_outputs() -> tuple[pd.DataFrame, dict[str, object], pd.DataFrame]:
    ensure_output_dir()
...
def _print_signal_query() -> None:
    _, signal_df, _ = generate_v2_2_outputs()
    row = signal_df.iloc[0]
    print("signal")
    print("strategy_version: v2.2")
    print("base_version: embedded_v2_base")
    print("signal_model: spread-NAV log-WLS exp half-life 6.0, lookback 17, no R2 gate")
    print(
...
def _print_realtime_signal_query() -> None:
    signal_df, meta, _ = build_realtime_v2_2_outputs()
    row = signal_df.iloc[0]
...
def _print_performance_query(query: str) -> None:
    ...
    v2_0.embedded_context.base_mod.STRATEGY_TITLE = "Top100 Microcap Mom16 Biweekly v2.2"
    try:
        v2_0.embedded_context.base_mod.build_performance_outputs(
            perf_df=perf_df,
            ret_col="return_net",
            nav_col="nav_net",
            source_label="costed_v2_2",
            query_text=query,
            paths={
...
def _handle_query(query: str) -> None:
    ...
    if stripped in {"信号", "信號", "signal", "淇″彿", "娣団€冲娇"}:
        _print_signal_query()
        return
    if stripped in {"实时信号", "實時信號", "realtime_signal", "live_signal", "瀹炴椂淇″彿", "鐎圭偞妞傛穱鈥冲娇"}:
    ...
    if v2_0.embedded_context.base_mod.PERFORMANCE_PATTERN.search(query):
        ...
        return
    raise ValueError("v2.2 supports: 信号 / 实时信号 / 表现 <区间>")
