...
def build_base_costed() -> tuple[dict[str, object], pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    official_summary, _official_signal, official_out = v2_0.generate_v2_0_outputs()
    ...
    base_gross = v2_0.embedded_context.base_mod.run_signal(close_df).sort_index()
    gross = v2_0.embedded_context.base_mod.apply_momentum_gap_exit_buffer(
        base_gross,
        float(v2_0.overlay_mod.V2_0_MOMENTUM_GAP_EXIT_BUFFER),
    )
    base_costed = v2_0.embedded_context.base_mod.apply_momentum_gap_no_peak_decay_cost_model(gross, turnover_df)
    scan_index = pd.DatetimeIndex(official_out.index.intersection(base_costed.index))
    scan_index = scan_index[scan_index >= FORMAL_START_DATE].sort_values()
    if scan_index.empty:
        raise RuntimeError("empty scan index")
    return official_summary, reference_summary, base_costed.loc[scan_index].copy(), turnover_df


def main() -> None:
    started = time.time()
    command = "python run_v20_target_vol_scan.py"
    with (RUN_DIR / "command_log.txt").open("a", encoding="utf-8") as f:
        f.write("\n# v2.0 target-vol level scan\n")
        f.write(command + "\n")

    print("refreshing official v2.0 outputs before target-vol scan...", flush=True)
    official_summary, reference_summary, base_costed, turnover_df = build_base_costed()

    rows: list[dict[str, object]] = []
    for target_vol in TARGET_VOLS:
        out = v2_0.overlay_mod.apply_target_vol_scaling(base_costed, target_vol=float(target_vol))
        params = {
            "candidate": f"v2_0_{label_target_vol(target_vol)}",
            "version": "v2.0",
    ...
    meta.update(
        {
            ...
            "baseline": {
                "candidate": f"v2_0_{label_target_vol(float(v2_0.overlay_mod.TARGET_VOL))}",
            ...
            "cost_model": {
                ...
                "target_vol_model": "v2_0.overlay_mod.apply_target_vol_scaling",
    ...
    default_row = window.loc[window["target_vol"].eq(float(v2_0.overlay_mod.TARGET_VOL))].iloc[0].to_dict()
    with (RUN_DIR / "record.md").open("a", encoding="utf-8") as f:
        ...
        f.write(f"- Official v2.0 latest nav date after refresh: `{official_summary.get('latest_nav_date', '')}`.\n")
        f.write("- Fixed parameters: official v2.0 gap buffer, cost model, 60-day realized-vol window, 1.5x max leverage, 10% scale rebalance threshold.\n")
